...
async function tryGroq(messages, abortSignal) {
  ...
  const res = await withTimeout(fetchCall, PROVIDER_TIMEOUT, 'Groq');
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    throw new Error(`Groq HTTP ${res.status}: ${body.slice(0, 160)}`);
  }
  const data = await res.json();
  if (!data?.choices?.[0]?.message?.content) throw new Error('Groq: unexpected response shape');
  return { reply: data.choices[0].message.content, model: 'Llama (Groq)' };
}
...
async function tryHuggingFace(messages, abortSignal) {
  ...
  const data = await res.json();
  if (!data?.choices?.[0]?.message?.content) throw new Error('HuggingFace: unexpected response shape');
